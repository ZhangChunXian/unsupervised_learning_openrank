...
请告诉我你需要哪方面的帮助，我会根据你的需求给出最合适的分析和建议！`,
  ...
  mutations: {
    ...
    appendLastMessageContent(state, content) {
      const lastMessage = state.messageList[state.messageList.length - 1];
      lastMessage.content += content;
    }
  },
  actions: {
    async sendChatMessage({ state, commit }, content) {
      try {
        if (state.messageList.length === 2) {
        ...
        const { body, status } = await chat(state.messageList, state.apiKey);
        ...
        if (body) {
          ...
          while (true) {
            ...
            if (status !== 200) {
              ...
              commit('appendLastMessageContent', content);
              return;
            }
        ...
        commit('appendLastMessageContent', error.toString());
      }
    }
  },
  modules: {}
