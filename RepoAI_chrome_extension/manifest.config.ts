...
export default defineManifest(async (env) => ({
  name:
    env.mode === 'staging'
      ? '[INTERNAL] RepoAI'
      : 'RepoAI - Power of ChatGPT on any website',
  description:
    "RepoAI is a browser extension that allows you to use Open AI's GPT in any website.",
  version: `${major}.${minor}.${patch}.${label}`,
  version_name: version,
  action: {
    default_title: 'RepoAI - Open Sidebar',
  },
  commands: {
    'open-sidebar': {
      suggested_key: {
